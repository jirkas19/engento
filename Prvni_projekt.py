    ...
    if prihlaseni[jmeno] == heslo:
        ...
        if vyber_textu.isnumeric():
            for i in range(len(TEXTS)):
                if i == int(vyber_textu)-1:
                    odstavec = TEXTS[int(vyber_textu)-1]
            if odstavec in TEXTS:
                nahradaradek = odstavec.replace("\n", " ")
                nahradacarka = nahradaradek.replace(",", "")
                nahrada = nahradacarka.replace(".", "")
                oddel_slov = nahrada.split(" ")
                for i in oddel_slov:
                    if i.isnumeric():
                    ...
                    if i != "":
                        ...
                        oddel_slov.remove(i)
                statistika["pocet_slov"] = len(oddel_slov)
                for i in oddel_slov:
